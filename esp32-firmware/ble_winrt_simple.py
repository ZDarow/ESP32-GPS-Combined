...
from bleak.backends.winrt import BleakClientWinRT
...
async def main():
    ...
    for _ in range(5):
        ...
        for d in devs:
            if d.name and TARGET_NAME in d.name:
                device = d
                print(f"Found: {d.name} [{d.address}] RSSI={getattr(d, 'rssi', 'N/A')}")
                break
        if device:
            break
    ...
    print(f"Connecting with WinRT backend...")
    client = BleakClient(
        device.address,
        timeout=20.0,
        pair=True,
        backend=BleakClientWinRT
    )
