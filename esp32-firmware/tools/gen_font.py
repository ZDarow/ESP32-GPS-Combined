#!/usr/bin/env python3
"""Generate 5x7 font data for SSD1306 OLED display."""

# Standard 5x7 font data (7 bytes per character, 5 bits wide, MSB first)
...
FONT_5X7 = [
    ...
    # 126: ~
    [0x08, 0x14, 0x08, 0x14, 0x08, 0x00, 0x00],
]

# Generate C array
print("static const uint8_t s_font_5x7[] = {")
for i, char in enumerate(FONT_5X7):
    hex_bytes = ", ".join(f"0x{b:02X}" for b in char)
    print(f"    {hex_bytes},  // {chr(32 + i)} ({(32 + i)})")
print("};")
print()
print(f"Total characters: {len(FONT_5X7)}")
print(f"Total bytes: {len(FONT_5X7) * 7}")
